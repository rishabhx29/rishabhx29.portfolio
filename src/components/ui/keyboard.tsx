...
import { loadBuffer, playSound as playClip } from "@/lib/audio";
import {
  IconArrowNarrowLeft,
  IconBrightnessDown,
  IconBrightnessUp,
...
function KeyboardProvider({
  ...
  // Preload the sprite through the shared audio module (sound is optional).
  useEffect(() => {
    if (!enableSound || !soundUrl) return;
    loadBuffer(soundUrl).catch(() => {
  ...
  const playSound = useCallback(
    (phase: KeyboardEventPhase, keyCode: string) => {
      ...
      void playClip(soundUrl, { offsetMs: startMs, durationMs });
    },
    [enableSound, soundUrl],
  );
